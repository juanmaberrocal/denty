<?php
class financePolicy {
	private static $table_name = 'finance_policies';
	...
	===============================*/
	// get about policy from a specific office
	...
	// get financing images from a specific financing page
	function getFromFinancingPage($financing_page_id, $limit = 4, $cols = array(), $ext_cols = array()){
		// if no specific columns were passed use defined public cols
		if (empty($cols)){
			$cols = self::$columns_public;
		}
		...
		// select defined columns
		// find through financing page
		// order by descending ids (newest first)
		// limit results
		return "SELECT ".$select_string." FROM ".self::$table_name." INNER JOIN financing_pages ON ".self::$table_name.".financing_page_id = financing_pages.id WHERE ".self::$table_name.".active=1 AND financing_pages.id='".$financing_page_id."' ORDER BY ".self::$table_name.".id DESC LIMIT ".$limit;
	}
